...
bool js_cocos2dx_ui_ScrollView_initPositionYet(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    cocos2d::ui::ScrollView* cobj = (cocos2d::ui::ScrollView *)(proxy ? proxy->ptr : NULL);
    JSB_PRECONDITION2( cobj, cx, false, "js_cocos2dx_ui_ScrollView_initPositionYet : Invalid Native Object");
    if (argc == 0) {
        cobj->initPositionYet();
        args.rval().setUndefined();
        return true;
    }
    
    JS_ReportError(cx, "js_cocos2dx_ui_ScrollView_initPositionYet : wrong number of arguments: %d, was expecting %d", argc, 0);
    return false;
}
...
void js_register_cocos2dx_ui_ScrollView(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec funcs[] = {
        ...
        JS_FN("initPositionYet", js_cocos2dx_ui_ScrollView_initPositionYet, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
...
bool js_cocos2dx_ui_RichElementText_enableOutline(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    bool ok = true;
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    cocos2d::ui::RichElementText* cobj = (cocos2d::ui::RichElementText *)(proxy ? proxy->ptr : NULL);
    JSB_PRECONDITION2( cobj, cx, false, "js_cocos2dx_ui_RichElementText_enableOutline : Invalid Native Object");
    if (argc >= 2) {
        cocos2d::Color4B arg0;
        int arg1 = 0;
        ok &= jsval_to_cccolor4b(cx, args.get(0), &arg0);
        ok &= jsval_to_int32(cx, args.get(1), (int32_t *)&arg1);
        JSB_PRECONDITION2(ok, cx, false, "js_cocos2dx_ui_RichElementText_enableOutline : Error processing arguments");
        cobj->enableOutline(arg0, arg1);
        args.rval().setUndefined();
        return true;
    }
...
void js_register_cocos2dx_ui_RichElementText(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec funcs[] = {
        ...
        JS_FN("enableOutline", js_cocos2dx_ui_RichElementText_enableOutline, 2, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FS_END
    };
