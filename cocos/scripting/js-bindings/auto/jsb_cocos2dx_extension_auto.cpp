...
bool js_cocos2dx_extension_AssetsManagerEx_getProgressInSize(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    cocos2d::extension::AssetsManagerEx* cobj = (cocos2d::extension::AssetsManagerEx *)(proxy ? proxy->ptr : NULL);
    JSB_PRECONDITION2( cobj, cx, false, "js_cocos2dx_extension_AssetsManagerEx_getProgressInSize : Invalid Native Object");
    if (argc == 0) {
        std::string ret = cobj->getProgressInSize();
        jsval jsret = JSVAL_NULL;
        jsret = std_string_to_jsval(cx, ret);
        args.rval().set(jsret);
        return true;
    ...
    JS_ReportError(cx, "js_cocos2dx_extension_AssetsManagerEx_getProgressInSize : wrong number of arguments: %d, was expecting %d", argc, 0);
    return false;
}
...
void js_register_cocos2dx_extension_AssetsManagerEx(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec funcs[] = {
        JS_FN("getProgressInSize", js_cocos2dx_extension_AssetsManagerEx_getProgressInSize, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
